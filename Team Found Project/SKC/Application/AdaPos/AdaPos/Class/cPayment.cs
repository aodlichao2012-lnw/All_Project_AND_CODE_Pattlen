...
namespace AdaPos.Class
{
    public class cPayment
    {
        ...
        public static void C_ADDxDisChgBill(decimal pcAmt, decimal pcDisChg, string ptDisChgTxt, decimal pcB4DisChg, int pnDisType, string ptRefCode = "", List<cmlProrateDT> paoProDT = null, int pnStaDis = 2)
        {
            ...
            try
            {
                ...
                // INSERT TPSTSalHDDis
                oSql.Clear();
                oSql.AppendLine("INSERT INTO " + cSale.tC_TblSalHDDis + " WITH(ROWLOCK) ");
                oSql.AppendLine("(");
                ...
                oSql.AppendLine(" ,FTXhdRefCode");      //*Arm 63-03-12 เลขที่อ้างอิง Redeem
                oSql.AppendLine(")");
                oSql.AppendLine("VALUES ");
                oSql.AppendLine("(");
                ...
                oSql.AppendLine(" ,'" + ptRefCode + "'");       //*Arm 63-03-12 เลขที่อ้างอิง Redeem
                oSql.AppendLine(")");
                ...
                {
                    //C_PRCxDisChgProrateDT(pcAmt, tDisChgTxt, pnDisType); //*Arm 63-06-05 Comment Code

                    if (paoProDT != null && paoProDT.Count >0) //*Arm 63-06-05
                    {
                        C_PRCxDisChgProrateDT(pcAmt, tDisChgTxt, pnDisType, ptRefCode, paoProDT, pnStaDis);
                    }
                    else
                    {
                        C_PRCxDisChgProrateDT(pcAmt, tDisChgTxt, pnDisType);
                ...
                {
                    // ใช้แต้มแลกส่วนลด
                    C_PRCxDisChgProrateDT(pcAmt, tDisChgTxt, pnDisType, ptRefCode, paoProDT);
                }
                //++++++++++
            }
            catch (Exception oEx)
        ...
        /// <param name="pnDisType">Dis Type 1 ลดแบบจำนวน,2 ลดแบบเปอร์เซ็น,3 ชาร์ตแบบจำนวน,4 ชาร์ตแบบเปอร์เซ็น </param>
        /// <param name="ptRefCode">*Arm 63-03-12 เลขที่อ้างอิง Redeem</param>
        /// <param name="paoProDT"></param>
        /// <param name="pnStaDis">*Arm 63-06-08 2:ส่วนลดท้ายบิล, 3:ส่วนลดท้ายบิลตามรายการ</param>
        private static void C_PRCxDisChgProrateDT(decimal pcAmt, string ptDisChgTxt, int pnDisType, string ptRefCode = "", List<cmlProrateDT> paoProDT = null, int pnStaDis = 2)
        {
            ...
            try
            {
                ...
                if (aoSalDT.Count > 0)
                {
                    ...
                    if (aoSalDT.Count == 1)
                    {
                        ...
                        //oSql.AppendLine(" ,GETDATE(),2,'" + ptDisChgTxt + "'"); //*Arm 63-06-08 Comment Code
                        oSql.AppendLine(" ,GETDATE(),"+ pnStaDis +",'" + ptDisChgTxt + "'"); //*Arm 63-06-08
                        ...
                        else
                        {
                            ...
                            if (oRep.StatusCode == System.Net.HttpStatusCode.OK)
                            {
                                ...
                                else
                                {
                                    ...
                                    new cLog().C_WRTxLog("cPayment", "C_PRCbCancelCoupon : " + oRes.rtDesc);
                                    return false;
                                }
                            }
                        }
