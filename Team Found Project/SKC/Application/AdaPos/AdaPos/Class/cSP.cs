...
namespace AdaPos.Class
{
    public class cSP
    {
        ...
        /// <summary>
        /// Net 63-05-22
        /// Get Json Text From API Method POST
        /// </summary>
        /// <param name="ptUrl"></param>
        /// <param name="ptJsonReq"></param>
        /// <param name="paoHeaders"></param>
        /// <returns></returns>
        public string SP_GETtJsonFromAPIPOST(string ptUrl, string ptJsonReq = "", List<KeyValuePair<string, string>> paoHeaders = null)
        {
            cClientService oCall;
            HttpResponseMessage oResponse;
            try
            {
                if (paoHeaders != null) oCall = new cClientService(paoHeaders);
                else oCall = new cClientService();

                oResponse = new HttpResponseMessage();
                oResponse = oCall.C_POSToInvoke(ptUrl, ptJsonReq);
                ...
                {
                    throw new Exception(oResponse.StatusCode.ToString());
                }
            }
            catch (Exception oEx)
            {
                new cLog().C_WRTxLog("cSP", MethodBase.GetCurrentMethod().Name + " : " + oEx.Message);
            }
            finally
            {
                SP_CLExMemory();
        ...
        public void SP_SHWxMsg(string ptMsg, int pnCaseIcon)
        {
            ...
            try
            {
                ...
                {
                    case 1:
                        MessageBox.Show(ptMsg, "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        break;

                    case 2:
                        MessageBox.Show(ptMsg, "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        break;

                    case 3:
                        MessageBox.Show(ptMsg, "", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        break;

                    case 4:
                        MessageBox.Show(ptMsg, "", MessageBoxButtons.OK, MessageBoxIcon.Question);
                        break;
