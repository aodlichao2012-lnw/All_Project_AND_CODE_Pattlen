...
namespace AdaPos.Popup.wSale
{
    public partial class wReferBill : Form
    {
        ...
        private int nW_RefMode;    //*Arm 63-05-23 1:ขายอ้างอิงบิลคืน, 2:คืนอ้างอิงขาย
        ...
        public wReferBill(int pnRefMode = 2)
        {
            ...
            try
            {
                nW_RefMode = pnRefMode; //*Arm 63-05-23
        ...
        private void W_SETxText()
        {
            try
            {
                ...
                // Menu
                if (nW_RefMode == 1) //*Arm 63-06-05
                {
                    olaTitleReferBill.Text = oW_Resource.GetString("tTitleReferBillRtn");
                }
                else
                {
                    olaTitleReferBill.Text = oW_Resource.GetString("tTitleReferBill"); //*Arm 62-12-26
                }
                ...
                otpSaleDate.Text = cVB.tVB_SaleDate; //*Arm 63-05-23
            }
            ...
            try
            {
                ...
                if(ockRefer.Checked)
                {
                    ...
                    else
                    {
                        ...
                        otbDocument.Focus();
                        cVB.nVB_ReferBy = 2;
                        otpSaleDate.Enabled = true; //*Arm 63-05-23
                        //*Arm 62-12-20
                        olaWristband.Enabled = false;
                ...
                else
                {
                    ...
                    otpSaleDate.Enabled = false; //*Arm 63-05-23
        ...
        private void ocmSchDoc_Click(object sender, EventArgs e)
        {
            try
            {
                ////this.Visible = false;
                //cVB.tVB_DocNo = "";
                //new wSearchDoc(Convert.ToDateTime(otpSaleDate.Value.ToString("yyyy-MM-dd")), nW_RefMode).ShowDialog();
        ...
        private int C_CHKnCheckConditionDoc()
        {
            ...
            try
            {
                ...
                //=================================================
                // 1. เอา PosCode จากเลขที่บิลที่ระบุ เริ่มตำแหน่งที่ 8 เอาแค่ 5 ตัว 
                // 2.Check PosCode ที่ได้จากเลขที่บิล 
                ...
                if (!string.IsNullOrEmpty(otbDocument.Text))
                {
                    ...
                    if (tPosCode == cVB.tVB_PosCode)  //*Arm 63-05-23 Check PosCode ที่ได้จากเลขที่บิล 
                    {
                        ...
                        cVB.bVB_RefundTrans = false; //ข้อมูลใน Temp(local)
                        nResult = W_SCHnSearchByLocal(false); // ค้นข้อมูลในตาราง Temp 

                        if (nResult > 0) // ถ้าไม่มีข้อมูลในตาราง Temp ให้หาข้อมูลจากตาราง Transaction
                        {
                        ...
                        {
                            cVB.bVB_RefundTrans = true; //ค้นข้อมูลใน Transaction(local)
        ...
        public bool W_PRCbInsert2Temp(cmlResItemSaleDwn poData)
        {
            ...
            cDataReaderAdapter<cmlTPSTSalDTPmtTmp> oPmt;
            ...
            try
            {
                ...
                // Bulk Copy : TPSTSalDTPmt
                aoPmt = C_PRCaListSalDTPmt(poData.roItem.aoTPSTSalDTPmt);
                oPmt = new cDataReaderAdapter<cmlTPSTSalDTPmtTmp>(aoPmt);
                using (SqlBulkCopy oBulkCopy = new SqlBulkCopy(cVB.oVB_ConnDB, SqlBulkCopyOptions.Default, oTranscation))
                {
                    foreach (string tColName in oPmt.ColumnNames)
                    {
                        oBulkCopy.ColumnMappings.Add(tColName, tColName);
                    }
                    ...
                    try
                    {
                        oBulkCopy.WriteToServer(oPmt);
                    }
                    catch (Exception oEx)
                    {
                        oTranscation.Rollback();
                        new cLog().C_WRTxLog("wReferBill", "W_PRCbInsert2Temp/TPSTSalDTPmtTmp : " + oEx.Message);
                        return false;
                    }
                }
        ...
        private List<cmlTPSTSalDTPmtTmp> C_PRCaListSalDTPmt(List<cmlResInfoSalDTPmt> paoSalDTPmt)
        {
            List<cmlTPSTSalDTPmtTmp> aoData = new List<cmlTPSTSalDTPmtTmp>();
            try
            {
                aoData = paoSalDTPmt.Select(oItem => new cmlTPSTSalDTPmtTmp()
                {
                    FTBchCode = oItem.rtBchCode,
                    FTXshDocNo = oItem.rtXshDocNo,
                    FTPmhCode = oItem.rtPmhCode,
                    FTXdpGrpName = oItem.rtXdpGrpName,
                    FTXsdBarCode = oItem.rtXsdBarCode,
                    FNXsdSeqNo = oItem.rnXsdSeqNo,
                    FCXdpQtyAll = oItem.rcXdpQtyAll,
                    FCXdpNet = oItem.rcXdpNet,
                    FCXdpSetPrice = oItem.rcXdpSetPrice,
                    FCXdpGetQtyDiv = oItem.rcXdpGetQtyDiv,
                    FCXdpGetCond = oItem.rcXdpGetCond,
                    FCXdpGetValue = oItem.rcXdpGetValue,
                    FCXdpDis = oItem.rcXdpDis,
                    FCXdpDisAvg = oItem.rcXdpDisAvg,
                    FCXdpPoint = oItem.rcXdpPoint,
                    FTXdpStaExceptPmt = oItem.rtXdpStaExceptPmt,
                    FTXdpStaRcv = oItem.rtXdpStaRcv

                }).ToList();
            }
            catch (Exception oEx) { new cLog().C_WRTxLog("wReferBill", "C_PRCaListSalDTPmt : " + oEx.Message); }
            finally
            {
                paoSalDTPmt = null;
                new cSP().SP_CLExMemory();
        ...
        #endregion End Insert Sale to Temp
