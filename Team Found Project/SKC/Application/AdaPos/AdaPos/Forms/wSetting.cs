...
namespace AdaPos.Forms
{
    public partial class wSetting : Form
    {
        ...
        private string tW_OldBch; //*Net 63-05-12
        private string tW_OldPos; //*Net 63-05-12
        private cmlRabbitMQConfig oW_OldMQ; //*Net 63-05-12
        ...
        private void W_CONxDatabase()
        {
            ...
            try
            {
                ...
                oCfg.tNameDB = ocbDBName.Text;

                cVB.oVB_ConnDB = new cDatabase().C_CONoDatabase(oCfg);

                if (cVB.oVB_ConnDB != null)
                {
                    using (SqlCommand oCmd = new SqlCommand("SELECT NAME FROM SYS.DATABASES", cVB.oVB_ConnDB))
                    {
                        if (cVB.oVB_ConnDB.State == ConnectionState.Closed) cVB.oVB_ConnDB.Open();
        ...
        private void ogdAPI_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            cmlTSysConfig oCfg;
            wEditUrl oFrmEdit;
            try
