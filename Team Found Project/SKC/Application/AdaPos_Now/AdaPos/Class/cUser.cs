    ...
    public class cUser
    {
        ...
        public List<cmlTCNMUser> C_GETaUser()
        {
            ...
            try
            {
                ...
                //oSql.AppendLine("SELECT USR.FTUsrCode, USL.FTUsrLogin AS FTUsrName, USL.FTUsrLoginPwd AS FTUsrPwd, AROL.FTRolCode, UGP.FTShpCode, UGP.FTUsrStaShop ");   //*Net 63-06-17 ใช้ตาราง  TCNMUsrActRole
                oSql.AppendLine("SELECT USR.FTUsrCode, USL.FTUsrLogin AS FTUsrName, USL.FTUsrLoginPwd AS FTUsrPwd, AROL.FTRolCode, UGP.FTShpCode, UGP.FTMerCode "); //*Em 63-06-18
                ...
                oSql.AppendLine("LEFT JOIN TCNMUsrActRole AROL WITH(NOLOCK) ON AROL.FTUsrCode = USR.FTUsrCode"); //*Net 63-06-17 ไม่ใช้ RolLevel แล้ว ใช้ FTRolCode จาก TCNMUsrActRole แทน
                ...
                oSql.AppendLine("   WHERE (UGP.FTBchCode = '" + cVB.tVB_BchCode + "' OR ISNULL(UGP.FTBchCode,'') = '')");   //*Em 62-06-19
        ...
        public List<cmlTCNMUser> C_GETaLastUser(string ptTypePwd)
        {
            ...
            try
            {
                ...
                //*Arm 63-08-15 End Comment Code


                //*Arm 63-08-15
                oSql.AppendLine("SELECT TOP(3) FTUsrCode, FTUsrName, FDSdtSignIn");
                oSql.AppendLine("FROM");
                oSql.AppendLine("(");
                oSql.AppendLine("   SELECT DISTINCT USRL.FTUsrCode, USRL.FTUsrName AS FTUsrName, MAX(SDT.FDSdtSignIn) AS FDSdtSignIn");
                oSql.AppendLine("   FROM TPSTShiftDT SDT WITH(NOLOCK)");
                oSql.AppendLine("   INNER JOIN TCNTUsrGroup UGP WITH(NOLOCK) ON SDT.FTUsrCode = UGP.FTUsrCode");
                //*Arm 63-08-26
                oSql.AppendLine("       AND (ISNULL(UGP.FTAgnCode, '') = '' OR ISNULL(UGP.FTAgnCode, '') = '" + cVB.tVB_AgnCode + "') ");
                oSql.AppendLine("       AND (ISNULL(UGP.FTBchCode, '') = '' OR ISNULL(UGP.FTBchCode, '') = '" + cVB.tVB_BchCode + "')");
                //+++++++++++++
                oSql.AppendLine("   INNER JOIN TCNMUser_L USRL WITH(NOLOCK) ON USRL.FTUsrCode = SDT.FTUsrCode AND USRL.FNLngID = '"+ cVB.nVB_Language +"'");
                oSql.AppendLine("   GROUP BY USRL.FTUsrCode, USRL.FTUsrName");
                oSql.AppendLine(") LastUsr");
                oSql.AppendLine("ORDER BY FDSdtSignIn DESC");
                //+++++++++++++
        ...
        /// <summary>
        /// Get User Name (Arm 63-08-15)
        /// </summary>
        /// <param name="ptUsrCode"></param>
        /// <param name="ptTypeLogin"></param>
        /// <returns></returns>
        public static string C_GETtUsrLogin(string ptUsrCode, string ptTypeLogin)
        {
            ...
            try
            {
                ...
                oSql.AppendLine("WHERE FTUsrCode = '" + ptUsrCode + "' AND FTUsrStaActive != '2' AND FTUsrLogType = '"+ ptTypeLogin + "' AND (CONVERT(VARCHAR(19),GETDATE(),121) BETWEEN CONVERT(VARCHAR(19),FDUsrPwdStart,121) AND CONVERT(VARCHAR(19),FDUsrPwdExpired,121)) ");
                return  new cDatabase().C_GEToDataQuery<string>(oSql.ToString());
            }
            catch(Exception oEx)
            {
                new cLog().C_WRTxLog("cUser", "C_GETtUsrLogin : " + oEx.Message);
                return "";
            }
            finally
            {
        ...
        public string C_CHKtUserRole(string ptRolCode, string ptScreen, string ptFuncCode) //*Net 63-06-17 เปลี่ยนการเช็คสิทธิ์ (1Userอาจมีหลายสิทธิ์)
        //public string C_CHKtUserRole(int pnRolLevel, string ptScreen, string ptFuncCode)    //*Em62-09-03
        {
            ...
            try
            {
                //*Net 63-06-17 เปลี่ยนการเช็คสิทธิ์ (1Userอาจมีหลายสิทธิ์)
                oSql = new StringBuilder();
                ...
                oSql.AppendLine("   AND FDT.FTSysCode = UFR.FTUfrRef");
                //oSql.AppendLine("WHERE UFR.FTRolCode = '" + ptRolCode + "'");
                oSql.AppendLine("WHERE UFR.FTRolCode IN (" + ptRolCode + ")"); //*Net 63-06-17 เปลี่ยนการเช็คสิทธิ์ (1Userอาจมีหลายสิทธิ์)
                oSql.AppendLine("AND UFR.FTUfrType = '1'");
        ...
        public string C_GETtUsername()
        {
            ...
            try
            {
                ...
                //tUsername = cVB.tVB_UsrName; //*Arm 63-08-11 Comment Code

                tUsername = cVB.tVB_ShwUsrName; //*Arm 63-08-11
            }
