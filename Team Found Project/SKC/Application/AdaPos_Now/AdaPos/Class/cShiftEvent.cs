...
namespace AdaPos.Class
{
    public class cShiftEvent
    {
        ...
        /// </summary>
        //public cmlTPSTShiftEvent C_GEToShiftEvent() //*Net 63-07-30 ปรับตาม Moshi
        public cmlTPSTShiftEvent C_GEToShiftSumEvent(string ptShfCode, string ptEvnCode)
        {
            StringBuilder oSql;
            ...
            try
            {
                ...
                oSql.AppendLine("SELECT ISNULL(SUM(FNSvnQty),0) AS FNSvnQty, ISNULL(SUM(FCSvnAmt),0) AS FCSvnAmt"); //*Net 63-07-30 เพิ่ม ISNULL
                oSql.AppendLine(" ,COUNT(*) AS FNSdtSeqNo"); //*Net 63-07-30 นับจำนวน
                ...
                oSql.AppendLine("WHERE FTEvnCode = '" + ptEvnCode + "'");
        ...
        //*Net 63-07-30
        public List<string> C_GETaShiftbyEvent(string ptEvnCode, string ptRmk = null)
        {
            StringBuilder oSql;
            ...
            {
                oSql = new StringBuilder();
                oSql.AppendLine("SELECT DISTINCT FTShfCode");
                oSql.AppendLine("FROM TPSTShiftEvent WITH(NOLOCK)");
                oSql.AppendLine("WHERE FTEvnCode = '" + ptEvnCode + "'");
                oSql.AppendLine("   AND FTBchCode = '" + cVB.tVB_BchCode + "'");
                ...
                if (ptRmk != null)
                {
                    oSql.AppendLine($"  AND FTSvnRemark = '{ptRmk}'");
                }

                return new cDatabase().C_GETaDataQuery<string>(oSql.ToString());
            }
            catch (Exception oEx) { new cLog().C_WRTxLog("cShiftEvent", "C_GEToShiftEvent : " + oEx.Message); }
            finally
            {
            ...
            return new List<string>(); //*Net 63-07-25 return เป็นว่าง
        }
        ...
        public void C_INSxShiftEvent(cmlTPSTShiftEvent poEvent)
        {
            ...
            {
                oSql = new StringBuilder();
                oSql.AppendLine("INSERT INTO TPSTShiftEvent ");//*Arm 63-05-26 เอา WITH(ROWLOCK) ออก
                oSql.AppendLine("(");
        ...
        /// UPDATE Shift Evene
        /// </summary>
        public void C_UPDxShiftEvent(string ptShfCode, string ptEvnCode, int pnQty, decimal pcAmt, string ptRmk = null)
        {
            StringBuilder oSql;
            ...
            {
                oSql = new StringBuilder();
                oSql.AppendLine("UPDATE TPSTShiftEvent ");
                oSql.AppendLine("SET");
                oSql.AppendLine("FNSvnQty=" + pnQty.ToString());
                oSql.AppendLine(", FCSvnAmt=" + new cSP().SP_SETtDecShwSve(2, pcAmt, cVB.nVB_DecSave));
                if (ptRmk != null)
                {
                    oSql.AppendLine($", FTSvnRemark='{ptRmk}'");
                }
                oSql.AppendLine("WHERE 1=1");
                oSql.AppendLine("AND FTEvnCode = '" + ptEvnCode + "'");
                oSql.AppendLine("AND FTShfCode = '" + ptShfCode + "'");
                oSql.AppendLine("AND FTBchCode = '" + cVB.tVB_BchCode + "'");
                oSql.AppendLine("AND FTPosCode = '" + cVB.tVB_PosCode + "'");
        ...
        public decimal C_GETcShiftBalance()
        {
            ...
            try
            {
                ...
                oSql.AppendLine($"FROM TSRC{cVB.tVB_PosCode} RC WITH(NOLOCK)");
                oSql.AppendLine($"INNER JOIN TSHD{cVB.tVB_PosCode} HD WITH(NOLOCK) ON RC.FTBchCode=HD.FTBchCode AND RC.FTXshDocNo=HD.FTXshDocNo");
                oSql.AppendLine("INNER JOIN TFNMRcv RCV ON RC.FTRcvCode=RCV.FTRcvCode AND RCV.FTFmtCode='001'");
                oSql.AppendLine($"WHERE HD.FTShfCode='{cVB.tVB_ShfCode}' AND  HD.FTBchCode='{cVB.tVB_BchCode}' AND HD.FTPosCode='{cVB.tVB_PosCode}' AND HD.FTXshStaDoc='1')");
                oSql.AppendLine("");
