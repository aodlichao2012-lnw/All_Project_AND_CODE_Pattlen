    ...
    public partial class wSyncData : Form
    {
        ...
        private void W_SETxText()
        {
            try
            {
                ...
                ockShfNow.Text = oW_Resource.GetString("tShiftNow"); //*Em 63-05-27
                ...
                //*Net 63-09-02 เลือกโหมด upload / export
                ocbPrcMode.Items.Clear();
                ocbPrcMode.Items.Add(oW_Resource.GetString("tUpload"));
                ocbPrcMode.Items.Add(oW_Resource.GetString("tExportBill"));
                ocbPrcMode.SelectedIndex = 0;
            }
        ...
        private void ocmOKSync_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (bW_Upload)
                {
                    //*Net 63-09-02 เรียก ExportBill
                    if (ocbPrcMode.SelectedIndex == 1)
                    {
                        W_PRCxExportBills();
                    }
                    else
            ...
            try
            {
                ...
                foreach (DataGridViewRow oRw in aoRow)
                {
                    ...
                    if (ockRecentUpdate.Checked)
                    {
                        ...
                        tDate = Convert.ToDateTime(oRw.Cells[2].Value).ToString("yyyy-MM-dd HH:mm:ss"); //*Em 63-08-11
                    }
                    else
                    {
                        tDate = odtDateSycn.Value.ToString("yyyy-MM-dd");
                    }
        ...
        private void W_SYNxUpload()
        {
            try
            {
                ...
                foreach (DataGridViewRow oRw in aoRow)
                {
                    ...
                    //*Em 63-05-27
                    if (oRw.Cells[3].Value.ToString() == "1")
                    {
                        cSale.C_PRCxAdd2TmpLogChg(nItemSelect, tDocNo, true);
                    }
                    else
                    {
                        cSale.C_PRCxAdd2TmpLogChg(nItemSelect, tDocNo, false);
                    }
                    //+++++++++++++++
                ...
                //*Em 63-05-27
                if (ocbType.SelectedValue.ToString() == "80")
                {
                    if (ockShfNow.Checked == true)
                    {
                        new cSyncData().C_PRCxSyncUld(true);
                    }
                    else
                    {
                        new cSyncData().C_PRCxSyncUld();
                ...
                //+++++++++++++++
                if (opgStatus.Value < opgStatus.Maximum) //*Net 63-07-31 Fix Error
                    opgStatus.Value += 1;
                opgStatus.PerformStep();
                opgStatus.Update();
        ...
        private void W_GETxUploadType()
        {
            ...
            try
            {
                ...
                if (aoSync_L != null)
                {
                    ...
                    {
                        ocbType.DataSource = null; //*Net 63-07-31 Fix Error
                        ocbType.Items.Clear();
                    }
                    ...
                    ocbType.DataSource = aoSync_L; //*Net 63-07-31 set proptery ก่อน แล้วค่อย set datasource
            ...
            try
            {
                ...
                switch (ptLogType)
                {
                    case "80": //การขาย-คืน
                        ...
                        {
                            ockShfNow.Enabled = false;
                        }
                        else
                        {
                            ockShfNow.Enabled = true;
                        }
                        if (ockShfNow.Checked == true)
                        {
                            oSql.AppendLine("SELECT ");
                            ...
                            oSql.AppendLine("'2' AS FTLogStaPrc,");
                            oSql.AppendLine("FDLastUpdOn AS FDLastUpdOn "); //*Net 62-12-27 ADD
                            oSql.AppendLine("FROM " + cSale.tC_TblSalHD + " WITH(NOLOCK) ");
                            oSql.AppendLine("WHERE CONVERT(varchar(10),FDXshDocDate,121) =  '" + odtDateSycnU.Value.ToString("yyyy-MM-dd") + "'");
                            oSql.AppendLine("AND FTBchCode = '" + cVB.tVB_BchCode + "' AND FTPosCode = '" + cVB.tVB_PosCode + "'");
                        ...
                        else
                        {
                            ...
                            oSql.AppendLine(ptLogType + "AS FNLogType,");
                            oSql.AppendLine("'' AS FTWahCode,");
                            oSql.AppendLine("'1' AS FTLogStaPrc,");
                            oSql.AppendLine("FDLastUpdOn AS FDLastUpdOn "); //*Net 62-12-27 ADD
                    ...
                    case "81":    //รอบการขาย
                        ...
                        oSql.AppendLine(ptLogType + "AS FNLogType,");
                        oSql.AppendLine("'' AS FTWahCode,");
                        oSql.AppendLine("'' AS FTLogStaPrc,");  //*Em 63-05-27
                        oSql.AppendLine("FDLastUpdOn AS FDLastUpdOn "); //*Net 62-12-27 ADD
                        oSql.AppendLine("FROM TPSTShiftHD WITH(NOLOCK) ");
                        oSql.AppendLine("WHERE CONVERT(varchar(10),FDShdSaleDate,121) =  '" + odtDateSycnU.Value.ToString("yyyy-MM-dd") + "'");
                        ockShfNow.Visible = false;  //*Em 63-05-27
                        break;
                    case "82":    //ยกเลิกบิล-ยกเลิกรายการ
                        ...
                        oSql.AppendLine("WHERE CONVERT(varchar(10),FDXihDocDate,121) =  '" + odtDateSycnU.Value.ToString("yyyy-MM-dd") + "'");
                        ockShfNow.Visible = false;  //*Em 63-05-27
                        break;
        ...
        private void ockShfNow_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                W_GETxTCNTTmpLogChg("80");
            }
            catch (Exception oEx) { new cLog().C_WRTxLog("wSyncData", "ockShfNow_CheckedChanged : " + oEx.Message); }
        }

        private void wSyncData_Shown(object sender, EventArgs e)
        {
        ...
        /// Export Bill
        /// </summary>
        private void W_PRCxExportBills()
        {
            ...
            try
            {
                ...
                if (aoRow == null || aoRow.Count == 0) return;
                opgStatus.Step = 0;
                opgStatus.Value = 0;
                opgStatus.Maximum = aoRow.Count + 1;
                opgStatus.Minimum = 0;
                ...
                using (var odgSaveFolder = new FolderBrowserDialog())
                {
                    DialogResult result = odgSaveFolder.ShowDialog();

                    if (result == DialogResult.OK && !string.IsNullOrEmpty(odgSaveFolder.SelectedPath))
                    {
                        tSavePath = odgSaveFolder.SelectedPath;
                    }
                    else return;
                }
                foreach (DataGridViewRow oRw in aoRow)
                {
                    ...
                    olaStatusTable.Text = tDocNo;
                    olaStatusTable.Update();

                    if (cExportSale.C_GENbExportBill(tDocNo, tSavePath, !ockShfNow.Checked)) nSuccess++;
                    opgStatus.Value = nCount;
                    opgStatus.PerformStep();
                    opgStatus.Update();
                }

                opgStatus.Value = opgStatus.Maximum;
                opgStatus.PerformStep();
                opgStatus.Update();

                if (opgStatus.Maximum - (nSuccess + 1) > 0)
                {
                    olaStatusTable.Text = oW_Resource.GetString("tExportBillError");
                    olaStatusTable.Update();
                    string tExport = $"{oW_Resource.GetString("tExportBillError")}\n";
                    tExport += $"{oW_Resource.GetString("tSuccess")} : {nSuccess}\n";
                    tExport += $"{oW_Resource.GetString("tUnSuccess")} : {opgStatus.Maximum - nSuccess - 1}";
                    new cSP().SP_SHWxMsg(tExport, 2);
                }
                else
                {
                    olaStatusTable.Text = oW_Resource.GetString("tExportBillSuccess");
                    olaStatusTable.Update();
                    new cSP().SP_SHWxMsg(oW_Resource.GetString("tExportBillSuccess"), 1);
                }
        ...
        /// <summary>
        /// Net 63-09-02 เปลี่ยนโหมด ให้เลือกเฉพาะบิลขายคืน
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void ocbPrcMode_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ocbPrcMode.SelectedIndex == 1)
            {
                ocbType.SelectedValue = "80";
