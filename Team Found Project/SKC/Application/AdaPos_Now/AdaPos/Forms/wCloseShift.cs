...
namespace AdaPos
{
    public partial class wCloseShift : Form
    {
        ...
        //public wBanknote oBankNote = new wBanknote(); //*Arm 63-09-08 Comment Code
        ...
        private void ocmBack_Click(object sender, EventArgs e)
        {
            try
            {
                if (Convert.ToDecimal(otbTotal.Text) > Convert.ToDecimal(0)) return;    //*Em 63-07-28
                new wHome().Show();
                otmClose.Start();
            }
        ...
        private void ocmNext_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                W_GETxPaymentType();
                W_INSxTPSTShiftSKeyBN();  //*Arm 63-09-08 เตรียมข้อมูล Bank Note
                cSale.C_OPNxCashDrawer(); //*Em 62-10-02

               //new Thread(()=>  cShift.C_CHKxShift2BackOffice()).Start(); //*Net 63-06-04
            }
        ...
        private void ocmAccept_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ////*Net 63-05-25 โอนจาก Temp ไป Transaction ย้าย
                //new cLog().C_WRTxLog("wCloseShift", "ocmAccept_Click : C_PRCxTemp2Transaction start");
                //cSale.C_PRCxTemp2Transaction();
                //new cLog().C_WRTxLog("wCloseShift", "ocmAccept_Click : C_PRCxTemp2Transaction end");
                ////+++++++++++++++++++++++++++++++++
                // C_CHKnShiftofSale() | ถ้าเป็น 1 = มียอดขาย | 2 = ไม่มียอดขาย
                //if (cVB.bVB_PrnShiftRCVRef)
                if (cVB.bVB_AlwChkShfRCV) //*Net 63-07-30
                {
                    //if (oShift.C_CHKnShiftofSale() == 1)
                    if (oShift.C_CHKnShiftofSale() > 0)
                    {
                        //if (otbTotal.Text == "0.00")
                        if (Convert.ToDecimal(String.IsNullOrEmpty(otbTotal.Text) ? "0" : otbTotal.Text) == 0m) //*Net 63-07-07 เช็คยอดจากค่า decimal
                        {
        ...
        private void W_PRCxShiftPrint()
        {
            ...
            if (otbPin.Enabled == false)
            {
                ...
                //W_INSxTPSTShiftSKeyBN(oBankNote.ogdBankNote); //*Arm 63-09-08 Comment Code

                if (cVB.bVB_PrnShiftBNK) //*Arm 63-09-08
                {
                ...
                W_FRMxDisableControl(); //*Em 63-07-28
                this.Refresh(); //*Em 63-07-28

                //*Net 63-05-25 โอนจาก Temp ไป Transaction ย้ายมาตอนกดปิดรอบ
                //new cLog().C_WRTxLog("wCloseShift", "ocmAccept_Click : C_PRCxTemp2Transaction start");
                //cSale.C_PRCxTemp2Transaction();
                //new cLog().C_WRTxLog("wCloseShift", "ocmAccept_Click : C_PRCxTemp2Transaction end");
                ////+++++++++++++++++++++++++++++++++
                ...
                //*Arm 63-05-28
                //wProgressUldCloseShif oPgs = new wProgressUldCloseShif();
                //oPgs.ShowDialog();
                //+++++++++++++

                //*Net 63-07-25 ปรับหน้าจอ Upload ใหม่
                using (wWaitUplCloseShf oWaitUpd = new wWaitUplCloseShf())
                {
                    oWaitUpd.ShowDialog();
                    //*Em 63-07-29
                    new cLog().C_WRTxLog("wCloseShift", "ocmAccept_Click : C_PRCxTemp2Transaction start", cVB.bVB_AlwPrnLog);
                    cSale.C_PRCxTemp2Transaction();
                    new cLog().C_WRTxLog("wCloseShift", "ocmAccept_Click : C_PRCxTemp2Transaction end", cVB.bVB_AlwPrnLog);
                    //++++++++++++++++
                }
        ...
        private void W_GETxPaymentType()
        {
            ...
            try
            {
                ...
                ////++++++++++++++++++
                oSql.AppendLine("LEFT JOIN TFNMRcvSpc APP WITH(NOLOCK) ON RCV.FTRcvCode = APP.FTRcvCode AND APP.FTAppCode = '" + tApp + "'"); //*Arm 63-08-10
        ...
        protected override void OnPaintBackground(PaintEventArgs e)
        {
            ...
            catch (Exception oEx) { new cLog().C_WRTxLog("wCloseShift", "OnPaintBackground : " + oEx.Message); }
        }
        ...
        /// <summary>
        /// เตรียมข้อมูล Bank Note (*Arm 63-09-08)
        /// </summary>
        ...
        private void W_INSxTPSTShiftSKeyBN()
        {
            ...
            try
            {
                //*Arm 63-09-08
                oSql = new StringBuilder();
                oSql.AppendLine("DELETE FROM TPSTShiftSKeyBN WITH(ROWLOCK) ");
                ...
                oSql.AppendLine("(FTBchCode, FTPosCode, FTShfCode, FNSdtSeqNo");
                oSql.AppendLine(",FTBntCode, FCKbnRateAmt, FNKbnQty, FCKbnAmt) ");
                oSql.AppendLine("");
                oSql.AppendLine("SELECT '" + cVB.tVB_BchCode + "' AS FTBchCode, '" + cVB.tVB_PosCode + "' AS FTPosCode, '" + cVB.tVB_ShfCode + "' AS FTShfCode, " + cVB.nVB_ShfSeq + " AS FNSdtSeqNo");
                oSql.AppendLine(", BNT.FTBntCode AS FTBntCode, BNT.FCBntRateAmt AS FCKbnRateAmt, 0 AS FNKbnQty, 0 AS FCKbnAmt");
                oSql.AppendLine("FROM TFNMBankNote BNT with(nolock)");
                oSql.AppendLine("LEFT JOIN TFNMBankNote_L BNTL with(nolock) ON BNT.FTRteCode = BNTL.FTRteCode AND BNT.FTBntCode = BNTL.FTBntCode AND BNTL.FNLngID = " + cVB.nVB_Language );
                ...
                e.RowIndex >= 0)
            {
                try
                {
                    ...
                    if (ogdPaymentType.CurrentRow.Cells[5].Value.ToString() == "001")
                    {
                        ...
                        //{
                            wBanknote oBankNote = new wBanknote(); //*Arm 63-09-08
                            oBankNote.ShowDialog();
                            if (oBankNote.DialogResult == DialogResult.OK)
                            {
                                //ogdPaymentType.CurrentRow.Cells[3].Value = new cSP().SP_SETtDecShwSve(1, Convert.ToDecimal(oBankNote.olaResult.Text), cVB.nVB_DecShow); //*Arm 63-09-08 Comment Code
                                ogdPaymentType.CurrentRow.Cells[3].Value = new cSP().SP_SETtDecShwSve(1, Convert.ToDecimal(oBankNote.rcSumBN), cVB.nVB_DecShow); //*Arm 63-09-08
                            }
                            //oBankNote.Dispose();
                            oBankNote.Hide();
                        //}
        ...
        {
            //W_SumAmount(); //*Arm 63-09-08 Comment Code
        }


        private void W_FRMxDisableControl()
        {
            try
            {
                this.BackColor = Color.DimGray;
                opnMenu.Visible = false;
                opnNotify.Visible = false;
                opnContent.Visible = false;
                opnHDMenu.Visible = false;
            }
            catch (Exception oEx)
            {
                new cLog().C_WRTxLog("wCloseShift", "W_FRMxDisableControl : " + oEx.Message);
            }
        }
    }
}
