...
namespace AdaPos.Popup.wPayment
{
    public partial class wRedeem : Form
    {
        ...
        private decimal cW_AmtB4BillDis = 0;   //*Arm 63-07-17
        ...
        protected override void OnPaintBackground(PaintEventArgs e)
        {
            ...
            catch (Exception oEx) { new cLog().C_WRTxLog("wRedeem", "OnPaintBackground " + oEx.Message); }
        }
                ...
                switch (pnMode)
                {
                    case 1:     //ประเภทเอกสาร 1: Redeem แต้ม+เงิน 
                        if (!string.IsNullOrEmpty(tW_CalType))
                        {
                            ...
                            oSql.AppendLine("AND DT.FTXsdStaAlwDis = '1' ");    //*Arm 63-07-17 ทำได้เฉพาะรายการที่อนุญาตเท่าน้้น
                    ...
                    case 2: //ประเภทเอกสาร 2: Redeem ส่วนลด
                        ...
                        //Arm 63-07-17 หามูลค่าก่อนลด ตามที่ Dicount Policy กำหนด
                        oDiscPolicy = new cDiscPolicy();
                        cW_AmtB4BillDis = oDiscPolicy.C_GETcGetAmtAlwDisc(2, cVB.tVB_StaPrice);

                        if (cW_AmtB4BillDis <= 0)
                        {
        ...
        public void W_PRCxDiscount()
        {
            ...
            cDatabase oDB;
            cDiscPolicy oDiscPolicy;    //*Arm 63-07-17
            ...
            try
            {
                oDiscPolicy = new cDiscPolicy(); //*Arm 63-07-17
                                ...
                                if (Convert.ToBoolean(oRow.Cells[ockChoose.Name].Value) == true)
                                {
                                    switch (oRow.Cells[otbColCalType.Name].Value.ToString()) // Check CalType 
                                    {
                                        case "1":   // CalType = การคำนวน 1: ส่วนลด(Default)
                                            ...
                                            ////*Arm 63-07-17
                                            //if (cVB.tVB_StaPrice == "1") //Full Net Price
                                            //{
                                            //    oSql.Clear();
                                            ...
                                            cB4Dis = oDiscPolicy.C_GETcGetAmtAlwDisc(3, cVB.tVB_StaPrice, (int)oRow.Cells[otbColSeq.Name].Value); // Get Amount ที่สามารถลดได้

                                            //*Arm 63-07-17 -ตรวจสอบมูลสินค้าต้อไม่น้อยกว่าส่วนลดที่ได้

                                            decimal cNet = oDiscPolicy.C_GETcGetAmount(3, (int)oRow.Cells[otbColSeq.Name].Value);
                                            if (cDiscount > cNet)
                                            {
                                            ...
                                            cVB.oVB_OrderRowIndex = cSale.nC_DTSeqNo - 1; //Row in Grid wSale
                        ...
                        else
                        {
                            switch (ogdRedeem.Rows[nW_SelectRow].Cells[otbColCalType.Name].Value.ToString()) // Check CalType : การคำนวน 1: ส่วนลด(Default)  2: เงินสด (ไม่ re-cal Vat)
                            {
                                case "1":   // CalType = การคำนวน 1: ส่วนลด(Default)
                                    ...
                                    //*Arm 63-07-17 มูลค่าสุทธิก่อนลดชาร์จ (มาจาก StaPrice ใน TSysDisPolicy 1: Full Net Price, 2: NetPrice)
                                    // 1: Sum(Qty*SetPrice)
                                    // 2: Sum(NetAfHD)
                                    cB4Dis = cW_AmtB4BillDis;

                                    //*Arm 63-07-18 Check มูลค่าส่วนลด ห้ามมากกว่ามูลค่าที่สามารถลดได้
                                    decimal cAmtDis = oDiscPolicy.C_GETcGetAmtAlwDisc(2, "2");
                                    if(cDiscount > cAmtDis)
                                    {
