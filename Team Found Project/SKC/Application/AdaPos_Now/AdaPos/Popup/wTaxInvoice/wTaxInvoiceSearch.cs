...
namespace AdaPos
{
    public partial class wTaxInvoiceSearch : Form
    {
        ...
        private void W_DATxLoadData()
        {
            ...
            try
            {
                oSql.AppendLine("SELECT TOP " + cVB.nVB_MaxData + " HD.FTXshDocNo,HD.FDXshDocDate,HD.FTPosCode,HD.FCXshGrand");
                //oSql.AppendLine("FROM TPSTSalHD WITH(NOLOCK)");
                oSql.AppendLine($"FROM {cSale.tC_TblSalHD} HD WITH(NOLOCK)"); //*Net 63-05-25 Search จาก ตาราง Tmp
                oSql.AppendLine("WHERE HD.FTBchCode = '" + cVB.tVB_BchCode + "' AND ISNULL(HD.FTXshDocVatFull,'') = ''");
                oSql.AppendLine("AND HD.FTXshStaDoc = '1'");   //*Net 63-05-25 เพิ่ม where StaDoc =1 
                if (!string.IsNullOrEmpty(otbTicketNo.Text))
                {
                    oSql.AppendLine("AND HD.FTXshDocNo LIKE '%" + otbTicketNo.Text.Trim() + "%'");
                }
                //*Em 62-10-08
                if (string.IsNullOrEmpty(cVB.tVB_Merchart))
                {
                    oSql.AppendLine("AND ISNULL(HD.FTShpCode,'') = ''");
                }
                else
                {
                    oSql.AppendLine("AND ISNULL(HD.FTShpCode,'') IN (SELECT FTShpCode FROM TCNMShop WITH(NOLOCK) WHERE FTMerCode = '"+ cVB.tVB_Merchart +"')");
                }
                //++++++++++++++++

                oSql.AppendLine("ORDER BY HD.FDCreateOn DESC");
                ...
                foreach (cmlTPSTSalHD oHD in aHD)
                {
                    ...
                    ogdSelectABB.Rows.Add(oHD.FTXshDocNo, oHD.FDXshDocDate, oHD.FTPosCode, new cSP().SP_SETtDecShwSve(1, (decimal)oHD.FCXshGrand, cVB.nVB_DecShow));    //*Em 63-06-07
                }
            }
