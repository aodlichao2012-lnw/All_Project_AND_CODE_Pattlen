    ...
    public partial class wOpenShift : Form
    {
        ...
        private void W_SETxDesign()
        {
            try
            {
                ...
                odtSaleDate.BackColor = cVB.oVB_ColLight;
                //++++++++++++++
            }
        ...
        private void W_SETxText()
        {
            try
            {
                ...
                odtSaleDate.Text = DateTime.Now.ToString("dd/MM/yyyy");
        ...
        private void W_PRCxChgSaleDate()
        {
            ...
            try
            {
                ...
                tChkRole = new cUser().C_CHKtUserRole(cVB.tVB_RolCode, cVB.tVB_KbdScreen, tFuncCode); //*Net 63-06-17
        ...
        private void W_PRCxOpenShift()
        {
            ...
            try
            {
                //bChkFormat = oW_SP.SP_CHKbDateFormat(odtSaleDate.Text);
                bChkFormat = oW_SP.SP_CHKbDateFormat(odtSaleDate.Value.ToString()); //*Arm 63-08-25

                if (bChkFormat)
                {
                    //dDate = DateTime.ParseExact(odtSaleDate.Text, "dd/MM/yyyy", null);
                    dDate = Convert.ToDateTime(odtSaleDate.Value); //*Arm 63-08-25
        ...
        private void W_OPNxChgSaleDate()
        {
            try
            {
                odtSaleDate.Value = DateTime.Now;
                odtSaleDate.Enabled = true;
                odtSaleDate.BackColor = Color.White; //*Arm 63-03-02
                //onpNumpad.Enabled = true;
                //onpNumpad.ocmDot.Enabled = false;
                //onpNumpad.oU_TextDateValue = odtSaleDate;
        ...
        private int W_CHKnDateOpenShift()
        {
            ...
            try
            {
                ...
                //oSql.AppendLine("WHERE CONVERT(VARCHAR(10),FDShdSignIn,121) > '" + Convert.ToDateTime(odtSaleDate.Text).ToString("yyyy-MM-dd") + "'"); //*Em 62-06-20
                oSql.AppendLine("WHERE CONVERT(VARCHAR(10),FDShdSignIn,121) > '" + Convert.ToDateTime(odtSaleDate.Value).ToString("yyyy-MM-dd") + "'"); //*Arm 63-08-26
