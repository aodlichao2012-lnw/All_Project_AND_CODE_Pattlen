...
namespace AdaPos.Popup.wSale
{
    public partial class wSearchDoc : Form
    {
        ...
        private DateTime dW_Date;   //*Arm 63-06-10
        private int nW_RefMode;     //*Arm 63-06-10 Mode 1:ขายอ้างอิงบิลคืน, 2:คืนอ้างอิงขาย

        /// <summary>
        /// 
        /// </summary>
        /// <param name="pdDate">รับค่า Date (*Arm 63-06-10)</param>
        ...
        public wSearchDoc(DateTime pdDate, int pnRefMode = 2)
        {
            ...
            try
            {
                ...
                dW_Date = pdDate;       //*Arm 63-06-10

                W_SETxDesign();
                W_SETxText();

                //Cursor.Current = Cursors.WaitCursor; //*Arm 63-06-10
                //W_SCHnSearchByAPI();    //* ค้นหาเอกสารทั้งหมดจากหลังบ้านภายใต้สาขา และภายในวันที่ที่เลือก ผ่าน API2ARDoc (*Arm 63-06-02)

                W_DATxLoadData();
                //Cursor.Current = Cursors.Default; //*Arm 63-06-10
            }
        ...
        private void W_SETxText()
        {
            try
            {
                ...
                W_SETxGridColumns(); //*Arm 63-06-10
            }
        ...
        private void W_DATxLoadData()
        {
            ...
            DataTable odtTmp = new DataTable(); //*Arm 63-06-10
            try
            {
                ...
                //oSql.AppendLine("SELECT TOP " + cVB.nVB_MaxData + " FTXshDocNo,FDXshDocDate,FTPosCode,FCXshGrand");
                ////oSql.AppendLine("FROM TPSTSalHD WITH(NOLOCK)");
                //oSql.AppendLine("FROM " + cSale.tC_TblSalHD + " WITH(NOLOCK)"); //*Em 63-05-25
                //oSql.AppendLine("WHERE FTBchCode = '" + cVB.tVB_BchCode + "' AND FNXshDocType = 1");
                //switch (cVB.nVB_ReturnType)
                //{
                //    case 1:   //1;คืนได้ครังเดียว เต็มบิลเท่านัน
                ...
                //}
                //oSql.AppendLine("AND FTXshDocNo LIKE '%" + otbTicketNo.Text.Trim() + "%'");
                //oSql.AppendLine("AND FTXshStaDoc = '1'");   //*Em 63-05-25
                //oSql.AppendLine("ORDER BY FDCreateOn DESC");
                ...
                //{
                //    //ogdDoc.Rows.Add(oHD.FTXshDocNo, oHD.FDXshDocDate, oHD.FTPosCode, oHD.FCXshGrand);
                //    ogdDoc.Rows.Add(oHD.FTXshDocNo, oHD.FDXshDocDate, oHD.FTPosCode, new cSP().SP_SETtDecShwSve(1, (decimal)oHD.FCXshGrand, cVB.nVB_DecShow));   //*Em 63-06-07
                //}



                //*Arm 63-06-10 ใช้ข้อมูล SaleHD จาก Temp / Transaction / Temp ที่ได้จาก API มา Union กัน แล้วแสดงใน Gridview
                //**************************************************************************************************
                ...
                if(odtTmp== null || odtTmp.Rows.Count == 0) //*Arm 63-07-22 แจ้งเตือนกรณีไม่พบเอกสาร
                {
                    new cSP().SP_SHWxMsg(cVB.oVB_GBResource.GetString("tMsgDataNotFound"), 3);
                    ocmBack_Click(ocmBack, null);
                }
        ...
        /// <summary>
        /// SET Column Gridview (*Arm 63-06-10)
        /// </summary>
        private void W_SETxGridColumns()
        {
            try
            {
                ...
                ogdDoc.Columns[otbTitleAmount.Name].DefaultCellStyle.Format = "###,###,##0." + new string('0', cVB.nVB_DecShow); //*Arm 63-08-10
            }
        ...
        private void ocmAccept_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                cVB.tVB_RefDocNo = tW_DocNo; //*Arm 63-05-25
        ...
        /// <summary>
        /// call API2ARDoc ค้นหาเอกสารการขายคืน (*Arm 63-06-10)
        /// </summary>
        public void W_SCHnSearchByAPI()
        {
            ...
            try
            {
                new cDatabase().C_PRCxCreateDatabaseTmp("TPSTSalHD", "TPSTSalHDTmp"); //*Arm 63-07-07
        ...
        public bool W_PRCbInsert2Temp(cmlResItemSaleDwn poData)
        {
            ...
            try
            {
                ...
                //new cDatabase().C_PRCxCreateDatabaseTmp("TPSTSalHD", "TPSTSalHDTmp"); //*Arm 63-07-07 Comment
        ...
        /// <summary>
        /// นำข้อมูลจาก cmlResInfoSalHD ลง cmlTPSTSalHDTmp (*Arm 63-06-10)
        /// </summary>
        /// <param name="paoSalHD"></param>
        /// <returns></returns>
