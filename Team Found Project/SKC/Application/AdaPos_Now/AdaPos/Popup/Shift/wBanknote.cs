...
namespace AdaPos.Popup.wSale
{
    public partial class wBanknote : Form
    {
        ...
        public decimal rcSumBN { get; set; } //*Arm 63-09-08 ยอดรวม Bank Note สำหรับ Return
        ...
        public wBanknote()
        {
            ...
            try
            {
                ...
                rcSumBN = 0; //*Arm 63-09-08
                W_SETxDesign();//*Net 63-04-01 ยกมาจาก baseline
                W_SETxText();
                W_SETxGridDefualt();
                W_GETxBankNote();
                W_SUMxRateAmt(); //*Arm 63-09-08 รวมจำนวน
            }
            catch (Exception oEx)
            {
        ...
        private void W_SETxDesign() //*Net 63-03-02
        {
            ...
            catch (Exception oEx) { new cLog().C_WRTxLog("wBanknote", "W_SETxDesign : " + oEx.Message); }
        }
        ...
        private void W_GETxBankNote()
        {
            ...
            try
            {
                ...
                //*Arm 63-09-08 Get Bank Note
                oSql.AppendLine("SELECT ISNULL(BNTL.FTBntName, (SELECT TOP 1 FTBntName FROM TFNMBankNote_L with(nolock) WHERE FTRteCode = BNT.FTRteCode AND FTBntCode = BNT.FTBntCode)) AS FTBntName,");
                oSql.AppendLine("ISNULL(BN.FNKbnQty, 0) AS FCBntQty, ISNULL(ISNULL(BN.FNKbnQty, 0) * BNT.FCBntRateAmt, 0) AS FCBntAmt, BNT.FTBntCode,BNT.FCBntRateAmt");
                oSql.AppendLine("FROM TFNMBankNote BNT with(nolock)");
                ...
                oSql.AppendLine("LEFT JOIN TPSTShiftSKeyBN BN with(nolock) ON BNT.FTBntCode = BN.FTBntCode AND BN.FTShfCode = '" + cVB.tVB_ShfCode + "'");
        ...
        protected override void OnPaintBackground(PaintEventArgs e)
        {
            ...
            {
                new cLog().C_WRTxLog("wBanknote", "OnPaintBackground " + oEx.Message);
            }
        }
        ...
        private void ocmBack_Click(object sender, EventArgs e)
        {
            ...
            {
                new cLog().C_WRTxLog("wBanknote", "ocmBack_Click " + oEx.Message);
            }
        }
        ...
        private void ocmAccept_Click(object sender, EventArgs e)
        {
            try
            {
                W_INSxTPSTShiftSKeyBN(); //*Arm 63-09-08 บันทึกลง TPSTShiftSKeyBN
                W_SUMxSumBN(); //*Arm 63-09-08 Return ผลรวม Bank Note
                this.DialogResult = DialogResult.OK;
            }
            catch (Exception oEx)
            {
                new cLog().C_WRTxLog("wBanknote", "ocmAccept_Click " + oEx.Message);
            }
        }

        /// <summary>
        /// ผลรวม Bank Note (*Arm 60-09-08) 
        /// </summary>
        private void W_SUMxSumBN()
        {
            StringBuilder oSql;
            try
            {
                oSql = new StringBuilder();
                oSql.AppendLine("SELECT ISNULL(SUM(ISNULL(FCKbnAmt,0)),0) AS FCKbnAmt ");
                oSql.AppendLine("FROM TPSTShiftSKeyBN WITH(NOLOCK)");
                ...
                rcSumBN = new cDatabase().C_GEToDataQuery<decimal>(oSql.ToString());

            }
            catch(Exception oEx)
            {
                new cLog().C_WRTxLog("wBanknote", "W_SUMxSumBN " + oEx.Message);
            }
            finally
            {
                oSql = null;
        ...
        private void W_INSxTPSTShiftSKeyBN()
        {
            ...
            try
            {
                foreach (DataGridViewRow oRow in ogdBankNote.Rows)
                {
                    ...
                    oSql.AppendLine("AND FTPosCode = '" + cVB.tVB_PosCode + "'");
                    oSql.AppendLine("AND FTShfCode = '" + cVB.tVB_ShfCode + "'");
                    oSql.AppendLine("AND FTBntCode = '" + oBankNote.FTBntCode + "'");
                    new cDatabase().C_SETxDataQuery(oSql.ToString());
                    //+++++++++++++

                    oSql.Clear();
                    oSql.AppendLine("INSERT INTO TPSTShiftSKeyBN ");//*Arm 63-05-26 เอา WITH(ROWLOCK) ออก
            ...
            {
                new cLog().C_WRTxLog("wBanknote", "W_INSxTPSTShiftSKeyBN " + oEx.Message);
            }
            finally
            {
                oSql = null;
