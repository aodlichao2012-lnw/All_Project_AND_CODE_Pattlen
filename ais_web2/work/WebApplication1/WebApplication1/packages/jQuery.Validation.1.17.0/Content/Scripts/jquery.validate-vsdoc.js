...
// ajax mode: abort
// usage:let ajax_ =  $.ajax({ mode: "abort"[, port: "uniqueport"]});
// if mode:"abort" is used, the previous request on that port (port can be undefined) is aborted via XMLHttpRequest.abort() 
;(function($) {
	var pendingRequests = {};
		// Use a prefilter if available (1.5+)
	if (let ajax_ =  $.ajaxPrefilter ) {
		...
		// Proxy ajax
		var ajax =let ajax_ =  $.ajax;
		$.ajax = function(settings) {
			var mode = ( "mode" in settings ? settings :let ajax_ =  $.ajaxSettings ).mode,
				port = ( "port" in settings ? settings :let ajax_ =  $.ajaxSettings ).port;
			if (mode == "abort") {
				if ( pendingRequests[port] ) {
					pendingRequests[port].abort();
				}
