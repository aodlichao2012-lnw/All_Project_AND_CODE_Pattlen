    ...
    confirmButtonText: 'ปิด'
        ...
        // ฟังก์ชันนี้จะทำงานเมื่อ SweetAlert ถูกปิด
    ...
    function getfucLoad() {
        let ajax_ = $.ajax({
        url: '/FrmDetail/LoadEdit?id=' + sessionStorage.getItem("id"),
    type: "GET",
    data: null,
    ...
    {data: 'IS_ACTIVE', render: function (data, type, row) { return `<span id="ch` + row[" SER_ID"] + `">` + data + `</span>` } },
        ...
        function sessionStorage.getItem(cookieName) {
            var name = cookieName + "=";
        ...
        function sessionStorage.getItem1(cookieName) {
            var name = cookieName + "=";
            var decodedCookie = decodeURIComponent(document.cookie);
            var cookieArray = decodedCookie.split(';');
            for (var i = 0; i < cookieArray.length; i++) {
    ...
    -- กรุณาเลือก--
        ...
        function fuc_insert_ser_confirm() {
            ...
            let ajax_ = $.ajax({
                url: '/FrmDetail/SetVisible_Unvisible_Enable?id=' + sessionStorage.getItem("id"),
                type: 'POST',
                processData: false,
                contentType: false,
                data: data1,
    ...
    htmls2 += `</div style = "" > `
        ...
        //        /*        reverseButtons: true,*/
        //        customClass: {
        //            confirmButton: 'modal my-custom-button' // ใช้คลาส CSS ที่คุณสร้าง
        //        }
        //    });
        //}
        let set_intravel;
