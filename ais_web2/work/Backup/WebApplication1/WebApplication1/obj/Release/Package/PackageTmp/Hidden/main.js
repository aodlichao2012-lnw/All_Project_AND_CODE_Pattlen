    ...
    $.ajax({
        ...
        , success: function (e) {
            ...
            table2 = $("#sms_select").dataTable({
                ...
                columns: [
                    { data: 'SER_ID', render: function (data, type, row) { return '<input type="checkbox" id="addSERVICE_' + data + '" value="' + data + '" class="editor-active">' } },
...
$("#sms_select tbody").on('click', 'input[type="checkbox"]', function () {
    ...
    $span.each(function () {
        ...
        if (isChecked) {
            if (spanText === "True") {
                $("#btn_sms_add_save").prop('disabled', true);
                $("#btn_ser_remove").prop('disabled', false);
            } else {
                $("#btn_sms_add_save").prop('disabled', false);
                $("#btn_ser_remove").prop('disabled', true);
            }
...
function fuc_insert_ser_confirm() {
    ...
    for (i = 0; i < checkedCount; i++) {
        let service_var = checkedvalues[i].id
        console.log(service_var)
        let variable_service_ = `#` + service_var;
        $(variable_service_).show();
        $("#lbsersh" + service_var).hide();
        $("#lbser" + service_var).show();
        visible_data += "" + service_var.replace(`add`, ``) + "" + ','
    }
    visible_data += "'"
    data1.append("VISIBLE", visible_data)
    data1.append("txtTel_No", $("#txt_tel").val())
    $.ajax({
        url: '/FrmDetail/SetVisible_Enable',
        type: 'POST',
        processData: false,
        contentType: false,
        data: data1,
...
$("#btn_sms_add_save").on('click', function (e) { fuc_insert_ser_confirm() })
